...
def get_courseID_by_code(conn: sqlite3.Connection, course_code: str) -> str:
...
def get_courseID_by_title(conn: sqlite3.Connection, course_title: str) -> str:
...
def get_coops(conn: sqlite3.Connection) -> list:
...
def get_courseIDs_by_filters(conn: sqlite3.Connection, filters: schemas.CourseFilters) -> list:
    ...
    if "prerequisites" in filters.__dict__ and filters.prerequisites is not None and len(filters.prerequisites) > 0:
        ...
        for prereq in filters.prerequisites:
            ...
            code = get_courseID_by_title(conn, prereq)
            if code is not None:
...
def get_course_info_by_id(conn: sqlite3.Connection, course_id: str) -> dict:
...
def get_course_description_by_id(conn: sqlite3.Connection, course_id: str) -> str:
...
def get_sectionIDs_by_filters(conn: sqlite3.Connection, filters: schemas.SectionFilters) -> list:
    ...
    if "meet_times" in filters.__dict__ and filters.meet_times is not None and len(filters.meet_times) > 0:
        ...
        for meet_time in filters.meet_times:
            ...
            params.extend([meet_time.day, meet_time.start_time, meet_time.end_time])
...
def get_section_info_by_id(conn: sqlite3.Connection, section_id: str) -> dict:
...
def get_upcoming_events(conn: sqlite3.Connection) -> list:
...
def get_event_dates_by_name(conn: sqlite3.Connection, event_name: str) -> list:
...
def get_student_basic_info(conn: sqlite3.Connection, student_id: int) -> dict:
...
def get_advisor_email(conn: sqlite3.Connection, advisor_name: str) -> str:
...
def get_student_course_history(conn: sqlite3.Connection, student_id: int) -> list:
...
def get_student_interests(conn: sqlite3.Connection, student_id: int) -> list:
...
def get_student_tracked_sections(conn: sqlite3.Connection, student_id: int) -> list:
...
def get_program_requirements_by_title(conn: sqlite3.Connection, program_title: str) -> list:
...
def get_data_with_hierarchy(conn: sqlite3.Connection, table: str, targetID: int) -> dict:
    ...
    if related_tables:
        for table in related_tables:
            entry[f"{table}"] = get_data_with_hierarchy(conn, table, targetID)
...
def get_data_with_hierarchy_string(conn: sqlite3.Connection, table: str, targetID: str) -> str:
    ...
    Warnings:
        This can produce very large results for highly-referenced tables (catalogs).
    """
    hierarchy_data = get_data_with_hierarchy(conn, table, targetID)
    return hierarchy_data_to_string(hierarchy_data)

def get_ids_by_field_value(conn: sqlite3.Connection, table: str, field: str, value: str) -> list:
...
def get_ids_by_parent(conn: sqlite3.Connection, table: str, targetID: str) -> list:
...
def get_students_by_advisor(conn: sqlite3.Connection, advisorID: str) -> list:
...
async def async_get_courseID_by_code(conn: aiosqlite.Connection, course_code: str) -> str:
    ...
    if " " in course_code:
        ...
        for i, char in enumerate(course_code):
            if char.isdigit():
                ...
                number = course_code[i:]
                break
    cur = await conn.execute("SELECT ID FROM Courses WHERE Department = ? AND Code = ?", (department, number))
    row = await cur.fetchone()
    return row[0] if row else None

async def async_get_courseID_by_title(conn: aiosqlite.Connection, course_title: str) -> str:
    ...
    Warnings: This is designed for a database with unique course titles (with the exception of "Cooperative Work Experience"). If you plan to use this for a database that may have multiple courses with the same title, consider implementing an alternative lookup method.
    """
    cur = await conn.execute("SELECT ID FROM Courses WHERE Name = ?", (course_title,))
    row = await cur.fetchone()
    return row[0] if row else None

async def async_get_course_info_by_id(conn: aiosqlite.Connection, course_id: str) -> dict:
    ...
    Returns:
        out (dict): Mapping of column names to values for the matched course with keys:
              ...
              Returns empty dict if no course found with that ID.
    """
    cur = await conn.execute("SELECT ID, Name, Department, Code, Credits, Requirements FROM Courses WHERE ID = ?", (course_id,))
    row = await cur.fetchone()
    if row is None:
        return {}
    cols = ["ID", "Name", "Department", "Code", "Credits", "Requirements"]
    return {cols[i]: row[i] for i in range(len(cols))}

async def async_get_coops(conn: aiosqlite.Connection) -> list:
    ...
    Returns:
        out (list[str]): List of course IDs (from `Courses.ID`) for all courses with the name "Cooperative Work Experience":
            ...
            Note: If no courses are found with that name, returns an empty list.
    """
    cur = await conn.execute("SELECT ID, Department, Code FROM Courses WHERE Name = 'Cooperative Work Experience'")
    results = await cur.fetchall()
    return [row[0] for row in results]

async def async_get_courseIDs_by_filters(conn: aiosqlite.Connection, filters: schemas.CourseFilters) -> list:
    ...
    if "prerequisites" in filters.__dict__ and filters.prerequisites is not None and len(filters.prerequisites) > 0:
        ...
        for prereq in filters.prerequisites:
            query += "c.Requirements LIKE ?"
            params.extend([f"%{prereq}%"])
            code = await async_get_courseID_by_code(conn, prereq) if isinstance(prereq, str) else None
            if code is not None:
                query += " OR c.Requirements LIKE ?"
                params.append(f"%{code}%")
            if prereq != filters.prerequisites[-1]:
...
async def async_get_course_description_by_id(conn: aiosqlite.Connection, course_id: str) -> str:
    ...
    Returns:
        out (str): Course description from Courses.Description column if found,
            "Course not found" if no course matches the ID.
    """
    cur = await conn.execute("SELECT Description FROM Courses WHERE ID = ?", (course_id,))
    row = await cur.fetchone()
    if row is None:
        return "Course not found"
    return row[0]

async def async_get_sectionIDs_by_filters(conn: aiosqlite.Connection, filters: schemas.SectionFilters) -> list:
    ...
    if "locations" in filters.__dict__ and filters.locations is not None and len(filters.locations) > 0:
        location_conditions = []
        for location in filters.locations:
            location_conditions.append("(s.Location LIKE ?)")
            params.append(location)
        query += " AND (" + " AND ".join(location_conditions) + ")"
    if "meet_times" in filters.__dict__ and filters.meet_times is not None and len(filters.meet_times) > 0:
        meet_time_conditions = []
        for meet_time in filters.meet_times:
            meet_time_condition = "(mt.Day = ? AND mt.StartTime = ? AND mt.EndTime = ?)"
...
async def async_get_section_info_by_id(conn: aiosqlite.Connection, section_id: str) -> dict:
    ...
    Returns:
        out (dict): Mapping of column names to values with keys: ID, Department, Code, Name,
              ...
              Returns empty dict if section not found.
    """
    cur = await conn.execute("SELECT s.ID, c.Department, c.Code, c.Name, s.SectionNum, s.Instructor, s.Method, s.Location, s.MaxSeats, s.SeatsLeft FROM Sections as s JOIN CoursesOffered as co ON s.ParentID = co.ID JOIN Courses as c ON co.CourseID = c.ID WHERE s.ID = ?", (section_id,))
    row = await cur.fetchone()
    if row is None:
        return {}
    cols = ["ID", "Department", "Code", "Name", "SectionNum", "Instructor", "Method", "Location", "MaxSeats", "SeatsLeft"]
    return {cols[i]: row[i] for i in range(len(cols))}

async def async_get_upcoming_events(conn: aiosqlite.Connection):
    ...
    Returns:
        out (list[dict]): List of dicts with keys "ID", "Name", "Description" for each
                    upcoming event. Returns empty list if no upcoming events found.
    """
    cur = await conn.execute("SELECT e.ID, e.Name, e.Description FROM Events as e JOIN EventDates as ed ON e.ID = ed.ParentID WHERE ed.Date >= date('now') GROUP BY e.ID")
    events = []
    rows = await cur.fetchall()
    for row in rows:
        events.append({"ID": row[0], "Name": row[1], "Description": row[2]})
    return events

async def async_get_event_dates_by_name(conn: aiosqlite.Connection, event_name: str) -> list:
    ...
    Returns:
        list[dict]: List of dicts with keys "Date", "StartTime", "EndTime", "Location"
                    ...
                    with that name exists, or empty list if event has no future dates.
    """
    cur = await conn.execute("SELECT ID FROM Events WHERE Name = ?", (event_name,))
    result = await cur.fetchone()
    if result is None:
        return ["Event not found"]
    cur = await conn.execute("SELECT Date, StartTime, EndTime, Location FROM EventDates WHERE ParentID = ? AND Date >= date('now')", (result[0],))
    event_dates = []
    rows = await cur.fetchall()
    for row in rows:
        event_dates.append({"Date": row[0], "StartTime": row[1], "EndTime": row[2], "Location": row[3]})
    return event_dates

async def async_get_student_basic_info(conn: aiosqlite.Connection, student_id: int) -> dict:
    ...
    Returns:
        out (dict): Profile information with keys: Name, Email, Advisor, GPA, CreditsEarned,
              ...
              ["No program of study"] if student has no programs.
    """
    cur = await conn.execute("SELECT s.Name, s.Email, a.Name, s.GPA, s.CreditsEarned FROM Students as s JOIN Advisors as a ON s.AdvisorID = a.ID WHERE s.ID = ?", (student_id,))
    row = await cur.fetchone()
    if row is None:
        return {}
    name, email, advisor, gpa, credits_earned = row
    cur = await conn.execute("SELECT p.Title, p.Description, p.CreditsRequired FROM ProgramsOfStudy as p JOIN StudentProgramsOfStudy as sp ON p.ID = sp.ProgramID WHERE sp.ParentID = ?", (student_id,))
    programs_rows = await cur.fetchall()
    programs_of_study = []
    for prow in programs_rows:
        programs_of_study.append({"Title": prow[0], "Description": prow[1], "CreditsRequired": prow[2]})
    if not advisor:
        advisor = "No advisor assigned"
    if not programs_of_study:
        programs_of_study = ["No program of study"]
    return {"Name": name, "Email": email, "Advisor": advisor, "GPA": gpa, "CreditsEarned": credits_earned, "ProgramsOfStudy": programs_of_study}

async def async_get_student_course_history(conn: aiosqlite.Connection, student_id: int) -> list:
    ...
    Returns:
        out (list[dict]): List of dicts with keys "CourseCode", "Name", "Grade" for each course.
                    Returns ["No courses taken"] if student has no course history.
    """
    cur = await conn.execute("SELECT c.Department, c.Code, c.Name, ct.Grade FROM Courses as c JOIN CoursesTaken as ct ON c.ID = ct.CourseID WHERE ct.ParentID = ?", (student_id,))
    rows = await cur.fetchall()
    course_history = []
    for row in rows:
        course_history.append({"CourseCode": str(row[0]) + " " + str(row[1]), "Name": row[2], "Grade": row[3]})
    if not course_history:
        course_history = ["No courses taken"]
    return course_history

async def async_get_student_interests(conn: aiosqlite.Connection, student_id: int) -> list:
    ...
    Returns:
        out (list[str]): Interests from Interests.Interest for the student.
                   Returns ["No interests specified"] if student has no interests.
    """
    cur = await conn.execute("SELECT Interest FROM Interests WHERE ParentID = ?", (student_id,))
    rows = await cur.fetchall()
    interests = [r[0] for r in rows]
    if not interests:
        interests = ["No interests specified"]
    return interests

async def async_get_student_tracked_sections(conn: aiosqlite.Connection, student_id: int) -> list:
    ...
    Returns:
        out (list[dict]): List of tracked sections with keys "CourseCode", "SectionNumber", "Name".
                    Returns ["No sections currently being tracked"] if student tracks no sections.
    """
    cur = await conn.execute("SELECT c.Department, c.Code, c.Name, s.SectionNum FROM Courses as c JOIN Sections as s JOIN CoursesOffered as co JOIN TrackedSections as ts ON c.ID = co.CourseID AND s.ParentID = co.ID AND s.ID = ts.SectionID WHERE ts.ParentID = ?", (student_id,))
    rows = await cur.fetchall()
    tracked_sections = []
    for row in rows:
        tracked_sections.append({"CourseCode": str(row[0]) + " " + str(row[1]), "SectionNumber": str(row[3]), "Name": str(row[2])})
    if not tracked_sections:
        tracked_sections = ["No sections currently being tracked"]
    return tracked_sections

async def async_get_program_requirements_by_title(conn: aiosqlite.Connection, program_title: str) -> list:
    ...
    Returns:
        out (list[str]): Requirement strings for the program: 
            Format for options:
                ...
                ["No requirements found"] if program has no requirements.
    """
    cur = await conn.execute("SELECT prc.ID FROM ProgramRequiredCourses as prc JOIN ProgramsOfStudy as p ON prc.ParentID = p.ID WHERE p.Title = ?", (program_title,))
    requirement_rows = await cur.fetchall()
    if not requirement_rows:
        return ["Program not found"]
    program_requirements = []
    for row in requirement_rows:
        c_options_cur = await conn.execute("SELECT c.Department, c.Code, c.Name FROM Courses as c JOIN ProgramRequiredCourseOptions as prco Join ProgramRequiredCourses as prc ON c.ID = prco.CourseID AND prc.ID = prco.ParentID WHERE prc.ID = ? AND prco.CourseID IS NOT NULL", (row[0],))
        c_options = await c_options_cur.fetchall()
        d_options_cur = await conn.execute("SELECT Elective FROM ProgramRequiredCourseOptions as prco Join ProgramRequiredCourses as prc ON prco.ParentID = prc.ID WHERE prc.ID = ? AND prco.CourseID IS NULL", (row[0],))
        d_options = await d_options_cur.fetchall()
        requirement = ""
        for option in c_options:
            requirement += "Department: " + str(option[0]) + ", Course Number: " + str(option[1]) + ", Title: " + str(option[2])
            if option != c_options[-1] or len(d_options) > 0:
...
async def async_insert_student_interests(conn: aiosqlite.Connection, student_id: int, interest: list[str]) -> str:
    ...
    counter = 0
    for item in interest:
        await conn.execute("INSERT INTO Interests (ParentID, Interest) VALUES (?, ?)", (student_id, item))
        counter += 1
    await conn.commit()
    return f"{counter} new interest(s) added"

async def async_insert_student_tracked_section(conn: aiosqlite.Connection, student_id: int, course_code: str, section_number: str) -> str:
    ...
    if " " in course_code:
        ...
        for i, char in enumerate(course_code):
            if char.isdigit():
                ...
                number = course_code[i:]
                break
    cur = await conn.execute("SELECT s.ID FROM Sections as s JOIN CoursesOffered as co JOIN Courses as c ON s.ParentID = co.ID AND co.CourseID = c.ID WHERE c.Department = ? AND c.Code = ? AND s.SectionNum = ?", (department, number, section_number))
    result = await cur.fetchone()
    if result is None:
        return "Section not found"
    section_id = result[0]
    cur = await conn.execute("SELECT ID FROM TrackedSections WHERE ParentID = ? AND SectionID = ?", (student_id, section_id))
    if await cur.fetchone() is not None:
        return "Section already being tracked"
    await conn.execute("INSERT INTO TrackedSections (ParentID, SectionID) VALUES (?, ?)", (student_id, section_id))
    await conn.commit()
    return "Section added to tracked sections"
